...
flowchart LR
  ...
  JPA --> H2["H2 开发数据库"]
  BE --> RSS["公开 RSS: InfoQ / OSChina / Hacker News"]
...
| 表 | 关键字段 | 说明 |
| --- | --- | --- |
| `app_users` | `id`, `username`, `display_name`, `password`, `role`, `avatar_color` | 用户与角色。`username` 用于登录，`display_name` 用于页面展示 |
...
| Method | API | 请求/参数 | 返回 |
| --- | --- | --- | --- |
| POST | `/api/auth/mock-login` | `username`, `password` | 当前用户和 mock token |
...
| PUT | `/api/users/me` | `displayName`, `password?` | 更新后的用户资料 |

权限策略：导师可查看所有任务；实习生只可查看、修改和删除自己负责的任务。MVP 使用 `X-User-Id` 请求头识别当前用户，避免在 48 小时任务中投入过多 JWT 细节。

演示账号由 `DataInitializer` 初始化。登录名为 `daoshiA`、`daoshiB`、`xiaozhao`、`xiaoli`、`xiaowang`、`xiaoliu`，密码统一为 `123456`，页面展示名分别为导师A、导师B、实习生小赵、实习生小李、实习生小王、实习生小刘。
...
- RSS 资讯：优先使用 InfoQ 中文、OSChina 开源社区，英文 Hacker News 作为技术兜底；不依赖 API Key，降低演示失败风险。
...
- 对 AI 生成内容进行人工取舍：登录采用 Mock 而非完整 JWT，资讯采用公开 RSS 而非需 Key 的 API，保证 MVP 更稳。
...
| 资讯 API Key 风险 | 第三方 API 可能需要密钥或限流 | 改用 InfoQ 中文、OSChina、Hacker News 等公开 RSS |
